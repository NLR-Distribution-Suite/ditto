...
class ConcentricCableEquipmentMapper(CymeMapper):
    ...
    def parse(self, row, equipment_file):
        ...
        return ConcentricCableEquipment.model_construct(
            name=name,
...
class GeometryBranchByPhaseEquipmentMapper(CymeMapper):
    ...
    def map_conductors(self, row, spacing_ids):
        conductor_names = {}
        conductor_names["A"] = row["CondID_A"]
        conductor_names["B"] = row["CondID_B"]
        conductor_names["C"] = row["CondID_C"]
        if "CondID_N1" in row:
            conductor_names["N"] = row["CondID_N1"]
        else:
            conductor_names["N"] = row["CondID_N"]

        conductor_objects = {}
        for conductor_key in conductor_names:
            conductor_name = conductor_names[conductor_key]
            if conductor_name != "NONE":
                conductor_objects[conductor_key] = self.system.get_component(
                    component_type=BareConductorEquipment, name=conductor_name
                )

        phase_A_conductor = conductor_objects.get("A")
        phase_B_conductor = conductor_objects.get("B")
        phase_C_conductor = conductor_objects.get("C")
        neutral_conductor = conductor_objects.get("N")
