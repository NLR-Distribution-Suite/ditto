...
class DistributionTransformerThreeWindingEquipmentMapper(CymeMapper):
    ...
    def map_pct_full_load_loss(self, row):
        ...
        Rpu_12 = (
            float(row["PrimaryToSecondaryZ1"])
            / 100
            / ((1 + float(row["PrimaryToSecondaryXR1"]) ** 2) ** 0.5)
        )
        Rpu_13 = (
            float(row["PrimaryToTertiaryZ1"])
            / 100
            / ((1 + float(row["PrimaryToTertiaryXR1"]) ** 2) ** 0.5)
        )
        Rpu_23 = (
            float(row["SecondaryToTertiaryZ1"])
            / 100
            / ((1 + float(row["SecondaryToTertiaryXR1"]) ** 2) ** 0.5)
        )

        R12 = (
            Rpu_12
            * (float(row["PrimaryVoltage"]) ** 2 * 1000)
            / float(row["PrimaryRatedCapacity"])
        )
        R13 = (
            Rpu_13
            * (float(row["PrimaryVoltage"]) ** 2 * 1000)
...
class ThreeWindingEquipmentMapper(CymeMapper):
    ...
    def map_resistance(self, row, winding_number):
        ...
        R12 = (
            Rpu_12
            * (float(row["SecondaryVoltage"]) ** 2 * 1000)
            / float(row["SecondaryRatedCapacity"])
        )
        R13 = (
            Rpu_13
            * (float(row["TertiaryVoltage"]) ** 2 * 1000)
            / float(row["TertiaryRatedCapacity"])
        )
        R23 = (
            Rpu_23
            * (float(row["TertiaryVoltage"]) ** 2 * 1000)
    ...
    def map_tap_positions(self, row, winding_number, network_row):
        ...
        if tap_location == "":
            return [1.0 for _ in range(num_phases)]
