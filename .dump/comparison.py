...
def redirect_and_solve(master_path: Path) -> None:
	...
	dss.Text.Command(f'Batchedit regcontrol..* enabled=false')
	dss.Text.Command(f'Batchedit Transformer..* taps=[1, 1]')
	dss.Text.Command(f'redirect "{master_path}"')
	...
	for name in dss.Capacitors.AllNames():
		...
		records.append(
			Record(
				name=name,
				category="capacitor",
				key=normalized_terminal_key("capacitor", buses[0], dss.CktElement.NumPhases(), None),
				buses=buses,
				phases=dss.CktElement.NumPhases(),
...
def compare_buses(original: dict[str, Any], converted: dict[str, Any]) -> dict[str, Any]:
	...
	matched.sort(
		key=lambda item: abs(item["avg_pu"]["delta"]) if item["avg_pu"]["delta"] is not None else 0.0,
		reverse=True,
	)
	return {
...
def build_report(original_summary: dict[str, Any], converted_summary: dict[str, Any]) -> dict[str, Any]:
	return {
		"circuit": {
			...
			"losses_kw": compare_scalar(original_summary["losses_kw"], converted_summary["losses_kw"]),
			...
			"num_buses": compare_scalar(original_summary["num_buses"], converted_summary["num_buses"]),
...
def main() -> None:
	...
	report = {
		"original": {
			key: value for key, value in original_summary.items() if key not in {"buses", "lines", "transformers", "loads", "capacitors"}
		},
		"converted": {
			key: value for key, value in converted_summary.items() if key not in {"buses", "lines", "transformers", "loads", "capacitors"}
		},
